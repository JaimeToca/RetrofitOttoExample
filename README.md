...
**When to use Otto?** <br/>
Otto is a great way to communicate between different components in your project (really interesting when you're working with an MVP architecture). Basically, events are sent through a bus system and the classes interested to those events have to suscribe to that bus. I recommend you play with Otto, but keep in mind that sometimes is not the best option, specially when it comes to nesting events, you can really get in troubles and become mad with the debugging.In this project, there are 2 modules (Rest and Entities) and once the information is downloaded the rest module needs to comunicate with the main activity (the one that will show the information ).<br/>
